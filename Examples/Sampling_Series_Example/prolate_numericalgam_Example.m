function gam = prolate_numericalgam_Example(prolate_dat, n)
%
...
% Todo: remove matlab dependency
    ...
    % prolate
    [v] = prolate_ev_Example(prolate_dat, [n], xx0);
    ...
    fun = @(y) reshape( besselj(prolate_dat.N+prolate_dat.p/2, prolate_dat.c*xmax*y(:)).*sqrt(prolate_dat.c *xmax *y(:)) .*y(:).^((prolate_dat.p+1)/2) .* prolate_ZernikeNorm_ex_Example(prolate_dat.p,prolate_dat.N, vec, y(:)) , size(y) );
    % integration:
