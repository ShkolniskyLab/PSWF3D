function [ ProlatesEval,alphavect ] = EvalProlGrid_fixedN_Example( PointsPolar, N,prolate_dat, iserr , prolate_dat_tmp,truncationparam,flag,L )
...
RadProlEv = prolate_ev_Example(prolate_dat, radindx, PointsPolar(:,3));
...
if flag == 1
    ...
    for j = 1:loc
            ...
            SphHarmEval(:,j) = EvalSpherHarmPoint_Example(N,Basis(j,2),PointsPolar(:,1),PointsPolar(:,2));
    ...
    for j = 1:loc
            ...
            if j==loc
                ...
                SphHarmEval(4.*[1:((length(PointsPolar)-(2*L-1))/4)]-3,j) = EvalSpherHarmPoint_Example(N,Basis(j,2),PointsPolar(4.*[1:((length(PointsPolar)-(2*L-1))/4)]-3,1),PointsPolar(4.*[1:((length(PointsPolar)-(2*L-1))/4)]-3,2));
    ...
    for j = 1:loc
        ...
        for k = (length(PointsPolar)-(2*L-1)+1):length(PointsPolar)
            ...
            SphHarmEval(k,j) = EvalSpherHarmPoint_Example(N,Basis(j,2),PointsPolar(k,1),PointsPolar(k,2));
