...
function [prolate_dat, iserr , prolate_dat_tmp] = prolate_crea_Example(c, D, N, minEigenvalRatio, matdim , prolate_crea_options)
%
...
%  
    ...
    [mat, vdiag, voffdiag] = prolate_diffop_mat_full_Example(prolate_dat.c, prolate_dat.p ,prolate_dat.N , prolate_dat.creaparam.matdim-1);
    ...
    if (isfixfirst==1)
        prolate_dat_tmp.cfs(:,1) = prolate_crea_fix_eigenvec_Example(mat, prolate_dat_tmp , 1);
    end
    ...
    gam0 = prolate_analyticgam_Example(prolate_dat_tmp, 0);         
    % Compute the rest of the eigenvalues through recurrsion    
    [ratios , ~] = prolate_crea_eigRatios_Example(prolate_dat_tmp);    
...
function v = prolate_crea_fix_eigenvec_Example(mat, prolate_dat , jj)
