...
function [vdiag, voffdiag] = prolate_diffop_mat_tridiag_Example(c,p,N,maxk)  
%
...
%
    ...
    for k=0:maxk
        vdiag(k+1,1) = prolate_diffop_mat_diag_element_Example(c,p,N,k);
    end
    for k=0:maxk-1
        voffdiag(k+1,1) = prolate_diffop_mat_offdiag_element_Example(c,p,N,k);
    end
...
function v = prolate_diffop_mat_offdiag_element_Example(c,p,N,k)
%
...
function v = prolate_diffop_mat_diag_element_Example(c,p,N,k)
%
% helper function for prolate_diffop_mat_tridiag.
...
%
    NN=N+p/2;
    if (NN==0)&&(k==0)
        v = -( prolate_diffop_mat_kappa_Example(p,N,k) +c^2/2 );
    else
        v = -( prolate_diffop_mat_kappa_Example(p,N,k) +c^2*(2*k*(k+1)+NN*(2*k+NN+1))/((2*k+NN)*(2*k+NN+2)) );
    end
end
...
function kap = prolate_diffop_mat_kappa_Example(p,N,k)
%
% helper function for prolate_diffop_mat_tridiag.
%
    NN = N+p/2;
