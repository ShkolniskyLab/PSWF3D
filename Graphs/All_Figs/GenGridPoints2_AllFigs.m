function [ points,weights ] = GenGridPoints2_AllFigs( thetaparam,rparam,rnormalization )
...
%%%% Generate nodes,weights for Gaussian quadrature (each axis
%%%% separately)%%%
phiquad = TrapzAtkinson_AllFigs(phiparam);
thetaquad = QuadRulePolarAtkinson_AllFigs(thetaparam);
rquad = rnormalization*LegndreQuadRecipies_AllFigs( rparam,0,1 );
...
for j1 = 1:length(rquad(1,:))
   ...
   for j2 = 1:length(thetaquad(1,:))
      ...
      for j3 = 1:length(phiquad(1,:))
         ...
         weights(counter,1) =(rnormalization^3).*(rquad(1,j1)^2)*sin(thetaquad(1,j2))*phiquad(2,j3)*rquad(2,j1)*thetaquad(2,j2) ;
