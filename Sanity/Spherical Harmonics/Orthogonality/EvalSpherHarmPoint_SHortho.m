function [ Val ] = EvalSpherHarmPoint_SHortho( N,m,phi,theta)
...
if m >= 0
    ...
    Val = EvalPTilde_SHortho(N,m,cos(theta)).*exp(1i.*m.*phi);

else
    
    m1 = abs(m);
    Val = ((-1)^m1)*EvalPTilde_SHortho(N,m1,cos(theta)).*exp((-1)*1i.*m1.*phi);
